...
import Client_PasswordUpdate from "../Client_PasswordUpdate";
...
const router = <BrowserRouter><Client_PasswordUpdate /></BrowserRouter>
...
describe("Password Update", () => {
    ...
    test('should render old password input', () => {
        render(router)
        const input1 = screen.getByPlaceholderText("Re-Enter Your New Password")
    ...
    test("confirm password should not be empty", async () => {
        ...
        fireEvent.click(updateButton)
        const message = await screen.findByText('Re-enter your password')
        expect(message).toBeInTheDocument()
    })

    test("new password and confirm password should be same", async () => {
        render(router)
        const updateButton = screen.getByRole("button")
        const oldPassword = screen.getByPlaceholderText("Enter Your Current Password")
        const newPassword = screen.getByPlaceholderText("Enter Your New Password")
        ...
        fireEvent.change(oldPassword, { target: { value: 'manjesh123!' } })
        fireEvent.change(newPassword, { target: { value: 'manjesh123@' } })
        fireEvent.change(confirmPassword, { target: { value: 'manjesh123' } })

        fireEvent.click(updateButton)
        const message = await screen.findByText('Password does not match !...')
        expect(message).toBeInTheDocument()
    })

    test("new password should be minimum of 8 character", async () => {
        render(router)
        const updateButton = screen.getByRole("button")
        const oldPassword = screen.getByPlaceholderText("Enter Your Current Password")
        const newPassword = screen.getByPlaceholderText("Enter Your New Password")
        ...
        fireEvent.change(oldPassword, { target: { value: 'manjesh123!' } })
        fireEvent.change(newPassword, { target: { value: 'manjes' } })


        fireEvent.click(updateButton)
        const message = await screen.findByText('Password should be minimum of 6 character')
        expect(message).toBeInTheDocument()
    })
